...
-- Create the Locales base entries from main table
INSERT INTO `locales_gossip_menu_option` (`menu_id`,`id`) SELECT `menu_id`,`id` FROM `gossip_menu_option` WHERE `option_broadcast_text`=0 AND `gossip_menu_option`.`menu_id` NOT IN (SELECT `menu_id` FROM `locales_gossip_menu_option`) AND `gossip_menu_option`.`id` NOT IN (SELECT `id` FROM `locales_gossip_menu_option`);
...
-- Create the Locales base entries from main table
INSERT INTO `locales_questgiver_greeting` (`entry`) SELECT `entry` FROM `questgiver_greeting` WHERE `entry` NOT IN (SELECT `entry` FROM `locales_questgiver_greeting`);
